...
I have also made modifications so that it deletes the downloads from your `os.TempDir()` or the one you set as `DataDir` in `~/.torrodle.json` after you have finished watching. Since I am only trying to get it in a working state, options will be added later to toggle keeping/deleting downloads.
